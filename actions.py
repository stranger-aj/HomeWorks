...
def copy_contact() :
    name_of_file = input("введите имя файла в который следует произвести копирование ( по умолчанию new_file.txt ): ")
    print_contacts()
    phonebook = open('phonebook.txt', 'r', encoding="UTF-8")
    new_file = open(name_of_file if name_of_file != "" else "new_file.txt", 'a', encoding="UTF-8")
    with phonebook, new_file :
        list_contacts = phonebook.read().rstrip().split("\n\n")
        number_of_contact = int(input("Введите номер контакта для копирования: "))
        while number_of_contact not in range(1, len(list_contacts)+1 ) :
            number_of_contact = int(input("Некорректное значение. Введите номер контакта: "))
        with phonebook, new_file :
...
def change_contact() :
    ...
    if found_contacts != [] :
        phonebook = open('phonebook.txt', 'r', encoding="UTF-8")
        list_contacts = phonebook.read().rstrip().split("\n\n")
        number_of_contact = int(input("Введите номер контакта для изменения: "))
